...
class WebBLE {
    ...
    constructor (runtime, extensionId, peripheralOptions, connectCallback, resetCallback = null) {
        ...
        this.onDiscoveredDevices = this.onDiscoveredDevices.bind(this);
    
        this.requestPeripheral();
    }

    /**
     * Receive a discovered device property from Electron main process.
     * @param {event} event - event from main process.
     * @param {JSON} deviceListJSON - list of device property.
     *  @param {object} deviceProperty - property of a device {deveceID, deviceName}.
     *  @param {string} deviceProperty.deviceID - ID of the device.
     *  @param {string} deviceProperty.deviceName - name of the device.
     */
    onDiscoveredDevices (event, deviceListJSON) {
        JSON.parse(deviceListJSON).forEach(device => {
            // Convert to format for runtime.
            device.peripheralId = device.deviceId;
            device.name = device.deviceName;
            this._availablePeripherals[device.peripheralId] = device;
        });
        this._runtime.emit(
    ...
    requestPeripheral () {
        ...
        ipcRenderer.on('discovered-bluetooth-devices', this.onDiscoveredDevices);
        this._availablePeripherals = {};
        this.requestDeviceIsRunning = true;
        navigator.bluetooth.requestDevice(this._peripheralOptions)
            ...
            .catch(e => {
                ipcRenderer.send('cancel-select-bluetooth-device'); // Cancel the discovery in main process.
                this._handleRequestError(e);
            })
            .finally(() => {
                ipcRenderer.removeListener('discovered-bluetooth-devices', this.onDiscoveredDevices);
     ...
     * @param {string} id - deviceId to connect.
     */
    connectPeripheral (id) {
        if (!this._device) {
            ipcRenderer.send('connect-bluetooth-device', id);
            return;
        }
